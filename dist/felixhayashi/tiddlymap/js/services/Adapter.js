...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); /* @preserve TW-Guard */
/*\
...
var Adapter = function () {
  ...
  _createClass(Adapter, [{
    ...
    key: 'getGraph',
    value: function getGraph(_ref) {
      var view = _ref.view,
          matches = _ref.matches,
          includeNeighboursOf = _ref.includeNeighboursOf;


      $tm.start('Assembling Graph');

      view = new _ViewAbstraction2.default(view);
      matches = matches || _utils2.default.getMatches(view.getNodeFilter('compiled'));
      var neighScope = parseInt(view.getConfig('neighbourhood_scope'));
      var typeWL = view.getEdgeTypeFilter('whitelist');
      var toWL = _utils2.default.getArrayValuesAsHashmapKeys(matches);
      var edges = this.getEdgesForSet(matches, toWL, typeWL);

      var nodeIdWL = void 0;
      if (view.isEnabled('filter_nodes_by_edge_types')) {
        nodeIdWL = new Set();
        Object.values(edges).forEach(function (edge) {
          nodeIdWL.add(edge.from);nodeIdWL.add(edge.to);
        });
      }
      ...
      if (neighScope) {
        var neighboutMatches = includeNeighboursOf ? matches.filter(includeNeighboursOf) : matches;
        var neighbours = this.getNeighbours(neighboutMatches, {
          steps: neighScope,
          view: view,
          typeWL: typeWL,
          addProperties: {
      ...
      if (nodeIdWL) {
        var nodesFilteredByEdges = _utils2.default.makeHashMap();
        Object.values(graph.nodes).forEach(function (node) {
          if (nodeIdWL.has(node.id)) {
            nodesFilteredByEdges[node.id] = node;
          }
        });
    ...
    key: 'makeLabel',
    value: function makeLabel(tObj) {

      var label = tObj.fields[$tm.field.nodeLabel];

      return label && $tm.field.nodeLabel !== 'title' ? this.wiki.renderTiddler('text/plain', env.path.nodeLabel, {
