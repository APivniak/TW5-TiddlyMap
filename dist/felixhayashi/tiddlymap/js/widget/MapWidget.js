...
var MapWidget = function (_Widget) {
  ...
  function MapWidget(parseTreeNode, options) {
    ...
    _utils2.default.bindTo(_this, ['constructTooltip', 'handleResizeEvent', 'handleClickEvent', 'handleCanvasKeyup', 'handleCanvasKeydown', 'handleCanvasScroll', 'handleExtraCanvasScroll', 'handleCanvasMouseMove', 'handleWidgetKeyup', 'handleWidgetKeydown', 'handleTriggeredRefresh', 'handleContextMenu']);
    ...
    _utils2.default.addTWlisteners({
      ...
      'tmap:tm-neighbourhood-reset-trace': function tmapTmNeighbourhoodResetTrace() {
        _this.initAndRenderGraph(_this.graphDomNode);
      }
    }, _this, _this);
  ...
  _createClass(MapWidget, [{
    ...
    value: function render(parent, nextSibling) {
      ...
      if (_utils2.default.isPreviewed(this)) {
        ...
        var downloadCanvas = this.document.createElement('canvas');
    ...
    value: function renderPreview(header, body) {
      ...
      var snapshotTRef = this.view.snapshotTRef;
      ...
      if (snapshotTObj) {
        ...
        var tree = _utils2.default.getTiddlerNode(this.view.getRoot());
        tree.children.push(_utils2.default.getTranscludeNode(snapshotTRef));
    ...
    value: function renderFullWidget(widget, header, body) {
      ...
      this.sidebar = _utils2.default.getFirstElementByClassName('tc-sidebar-scrollable', undefined, false);
    ...
    value: function update(updates) {
      ...
      || changedTiddlers[this.view.getRoot()] // view's main config changed
          ...
          this.trace = _utils2.default.makeHashMap();
    ...
    value: function rebuildGraph() {
      ...
      if (resetFocus) {
        var preventZoom = this.isPreventZoomOnNextUpdate == null ? false : typeof this.isPreventZoomOnNextUpdate === 'number' ? this.isPreventZoomOnNextUpdate > Date.now() : this.isPreventZoomOnNextUpdate;
    ...
    value: function rebuildGraphData() {
      ...
      var params = {
        view: this.view,
        matches: _utils2.default.getMatches(this.view.getNodeFilter('compiled'))
      };

      if (this.view.isEnabled('neighbourhood_trace_clicks')) {
        var originalMatches = params.matches;
        var clickPathMatches = Object.keys(this.trace);
        var combinedMatches = [].concat(_toConsumableArray(originalMatches.filter(function (tRef) {
          return !_this3.trace[tRef];
        })), _toConsumableArray(clickPathMatches));
        params.matches = combinedMatches;
        params.includeNeighboursOf = this.view.isEnabled('neighbourhood_include_traced_node_neighbours') ? function (tRef) {
          return combinedMatches.includes(tRef);
        } : function (tRef) {
          return originalMatches.includes(tRef);
        };
      }

      var graph = $tm.adapter.getGraph(params);
    ...
    value: function initAndRenderGraph(parent) {
      ...
      this.addGraphButtons({
        'fullscreen-button': function fullscreenButton() {
          _this4.toggleEnlargedMode('fullscreen');
        },
        'halfscreen-button': function halfscreenButton() {
          _this4.toggleEnlargedMode('halfscreen');
        }
      });
      ...
      if (this.isLiveView() && this.view.isEnabled('neighbourhood_trace_clicks')) {
        ...
        this.trace[_utils2.default.getText(this.refreshTriggers[0])] = true;
      }
    }
  }, {
        ...
        if (ev.keyCode === 88) {
          ...
          if (this.conVector.from && this.conVector.to) {
            ...
            this.handleConnectionEvent(this.conVector, function () {
              ...
              _this5.conVector = { from: null, to: null };
            });
          }
        }
      } else {
    ...
    value: function handleContextMenu(ev) {
      ...
      this.contextMenu.show(selectedNodes, function (selectedNodes, div) {
        ...
        _utils2.default.registerTransclude(_this6, 'contextMenuWidget', tRef);
        _this6.contextMenuWidget.setVariable('mode', mode);
        _this6.contextMenuWidget.render(div);
      });
    }
  }, {
    ...
    value: function getVisOptions() {
      ...
      var _ref2 = localOptions.layout || {},
          hierarchical = _ref2.hierarchical;

      var corrections = {
        layout: {
          hierarchical: {
            enabled: hierarchical === undefined || hierarchical === null ? false : typeof hierarchical === 'boolean' ? hierarchical : hierarchical.enabled !== false
          }
        }
      };
      ...
      var options = _utils2.default.merge({}, globalOptions, _utils2.default.merge(localOptions, corrections));
      ...
      options.manipulation.deleteNode = function (data, callback) {
        _this7.handleRemoveElements(data);
        _this7.resetVisManipulationBar(callback);
      };
      ...
      options.manipulation.addEdge = function (data, callback) {
        _this7.handleConnectionEvent(data);
        _this7.resetVisManipulationBar(callback);
      };

      options.manipulation.addNode = function (data, callback) {
        _this7.handleInsertNode(data);
        _this7.resetVisManipulationBar(callback);
      };

      options.manipulation.editNode = function (data, callback) {
        _this7.handleEditNode(data);
        _this7.resetVisManipulationBar(callback);
      };
    ...
    value: function handleCreateView() {
      ...
      $tm.dialogManager.open('createView', args, function (isConfirmed, outTObj) {
        ...
        var newView = new _ViewAbstraction2.default(label, {
          isCreate: true,
          protoView: isClone ? _this8.view : null
        });

        _this8.setView(newView);
      });
    }
  }, {
    ...
    value: function handleRenameView() {
      ...
      $tm.dialogManager.open('renameView', args, function (isConfirmed, outTObj) {
        ...
        if (!label) {
          ...
          _this9.view.rename(label);
          _this9.setView(_this9.view);
        }
      });
    }
  }, {
    ...
    value: function handleEditView() {
      ...
      $tm.dialogManager.open('configureView', args, function (isConfirmed, outTObj) {
        ...
        var prvBg = _this10.view.getConfig('background_image');

        _this10.view.setConfig(config);
        if (config['physics_mode'] && !_this10.view.isEnabled('physics_mode')) {
          // when not in physics mode, store positions
          // to prevent floating afterwards
          _this10.view.saveNodePositions(_this10.network.getPositions());
        }

        var curBg = _this10.view.getConfig('background_image');
        ...
        _this10.view.setNodeFilter(nf);
        _this10.view.setEdgeTypeFilter(eTf);
      });
    }
    ...
    value: function handleSaveCanvas() {
      ...
      var imageName = this.view.getLabel() + '.png';
      ...
      $tm.dialogManager.open('saveCanvas', args, function (isConfirmed, outTObj) {
        ...
        var width = outTObj.fields.width || args.dialog.preselects.width;
        var height = outTObj.fields.height || args.dialog.preselects.height;

        // save the image with the new sizes defined by the user in the dialog
        _this11.createAndSaveSnapshot(width, height, tempImagePath);

        // allow the user to override the default name or if name is
        // empty use the original default name
        var title = outTObj.fields.name || args.dialog.preselects.imageName;
        var action = outTObj.fields.action;

        if (action === 'download') {
          _this11.handleDownloadSnapshot(width, height, title);
        } else if (action === 'wiki') {
          _utils2.default.cp(tempImagePath, title, true);
          _this11.dispatchEvent({
            type: 'tm-navigate', navigateTo: title
          });
        } else if (action === 'placeholder') {
          _this11.view.addPlaceholder(tempImagePath);
        }
    ...
    value: function handleDownloadSnapshot(width, height, title) {

      var a = this.document.createElement('a');
      var label = this.view.getLabel();
      a.download = title;
      a.href = this.getCanvasAsBase64({ size: { width: width, height: height } });
    ...
    value: function createAndSaveSnapshot(width, height, tRef, title) {
      $tw.wiki.addTiddler(new $tw.Tiddler({
        title: title || tRef,
        type: 'image/png',
        text: this.getCanvasAsBase64({ size: { width: width, height: height }, withoutPreamble: true })
      }, $tw.wiki.getCreationFields(), $tw.wiki.getModificationFields()));

      return tRef;
    }
  }, {
    key: 'getCanvasAsBase64',
    value: function getCanvasAsBase64() {
      var _ref3 = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
          withoutPreamble = _ref3.withoutPreamble,
          size = _ref3.size;

      var oldWidth = this.graphDomNode.style.width;
      var oldHeight = this.graphDomNode.style.height;
      if (size) {
      ...
      return withoutPreamble ? _utils2.default.getWithoutPrefix(data, 'data:image/png;base64,') : data;
    }
  }, {
    ...
    value: function handleDeleteView() {
      ...
      this.openStandardConfirmDialog(function (isConfirmed) {
        ...
        _this12.view.destroy();
        _this12.setView($tm.misc.defaultViewLabel);
        var msg = 'view "' + viewname + '\' deleted';
        _this12.logger('debug', msg);
        $tm.notify(msg);
      }, message);
    }
    ...
    value: function handleRemoveElements(_ref4) {
    ...
    value: function handleRemoveNodes(nodeIds) {
      ...
      $tm.dialogManager.open('deleteNodeDialog', params, function (isConfirmed, outTObj) {
        ...
        for (var i = nodeIds.length; i--;) {
          var success = _this13.view.removeNode(nodeIds[i]);
          if (success) {
            deletionCount++;
          }
        }
        ...
        // prevent zoom
        _this13.isPreventZoomOnNextUpdate = true;
    ...
    value: function handleSetCentralTopic(_ref5) {
    ...
    value: function handleInsertNode(node) {
      ...
      $tm.dialogManager.open('addNodeToMap', {}, function (isConfirmed, outTObj) {
        ...
        var tRef = _utils2.default.getField(outTObj, 'draft.title').trim();
    ...
    value: function handleEditNode(node) {
      ...
      var globalDefaultNodeStyle = $tm.config.vis.nodes;
      var localDefaultNodeStyle = _utils2.default.parseJSON(this.view.getConfig('vis'), {}).nodes;
      var nodes = {};
      nodes[node.id] = node;
      var nodeStylesByTRef = $tm.adapter.getInheritedNodeStyles(nodes);
      var groupNodeStyles = nodeStylesByTRef[tRef];
      var globalIndividualNodeStyle = _utils2.default.merge({}, { color: tObj.fields['color'] }, _utils2.default.parseJSON(tObj.fields['tmap.style']));
      ...
      var localIndividualStyle = _extends({}, this.view.getNodeData(node.id));
      ...
      addToPreselects('local', localIndividualStyle, ['label', 'tw-icon', 'fa-icon', 'open-view']);
      ...
      $tm.dialogManager.open('editNode', args, function (isConfirmed, outTObj) {
        ...
        var data = _utils2.default.parseJSON(fields['local-individual-node-style'], {});
